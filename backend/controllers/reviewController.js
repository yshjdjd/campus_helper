...
const reviewController = {
  async create(req, res) {
    ...
    try {
      ...
      if (rating >= 3) {
        await conn.execute('UPDATE users SET credit_score = credit_score + 1.0 WHERE id = ?', [reviewee_id]);
        await conn.execute('UPDATE users SET credit_score = credit_score + 0.5 WHERE id = ?', [reviewer_id]);
      }
