    ...
    <el-card>
      ...
      <div class="info">
        <span class="user-link" @click="router.push(`/users/${task.publisher_id}`)">
          <el-avatar :size="24" :src="task.publisher_avatar ? `http://localhost:3000${task.publisher_avatar}` : ''" />
          发布者：{{ task.publisher_name }}
        </span>
        <span v-if="task.acceptor_name" class="user-link" @click="router.push(`/users/${task.acceptor_id}`)">
          <el-avatar :size="24" :src="task.acceptor_avatar ? `http://localhost:3000${task.acceptor_avatar}` : ''" />
          接单者：{{ task.acceptor_name }}
        </span>
      ...
      <div v-if="task.status === 'in_progress' && isParticipant" class="confirm-status">
        ...
        <el-tag :type="task.acceptor_confirmed ? 'success' : 'info'" effect="plain" style="margin-left:8px">
          接单者 {{ task.acceptor_confirmed ? '已确认 ✓' : '未确认' }}
        </el-tag>
      </div>
      ...
      <div class="actions" v-if="authStore.isLoggedIn">
        ...
        <el-button v-if="task.status === 'in_progress' && task.acceptor_id === authStore.user?.id" type="warning" @click="handleAbandon">放弃</el-button>
...
const isParticipant = computed(() => {
  ...
  return task.value.publisher_id === authStore.user.id || task.value.acceptor_id === authStore.user.id
})

const hasConfirmed = computed(() => {
  if (!task.value || !authStore.user) return false
  if (task.value.publisher_id === authStore.user.id) return task.value.publisher_confirmed
  if (task.value.acceptor_id === authStore.user.id) return task.value.acceptor_confirmed
  return false
})
